...
| **Demoed live, partly simulated** | The **physical device beat**. `device-s22` is a real Galaxy S22 reporting real scheduler contention, and when it is oversubscribed the agent genuinely detects it, reasons over it via 0G Compute, and the policy demands a cross-operator quorum. **That quorum is one real World ID scan (opA) plus two simulated signers (opB, opC)** — we have one verified human, and the phone's blast radius spans three operators. The simulated signers are `ALLOW_SIMULATED_APPROVALS` fixtures; they reach the chain as `HumanApproval` events and are indistinguishable on-chain from a real one. Say so if the phone quorum is shown. The unfudged personhood claim is `recurring_fault`, above. |
| **Never say** | that two humans scanned live, or that the on-chain two-nullifier override was two people. It was seeded — see the ⚠️ under *To paste before submitting*. Likewise never present the phone quorum as three humans. |
